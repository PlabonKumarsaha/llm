...
async function main(q = '') {
  ...
  let hasInteruption = result.interruptions.length > 0;
    ...
    for (const interput of result.interruptions) {
      if (interput.type === 'tool_approval_item') {
       const isAllowed = await askForUserConfitmation(
          `Agent ${interput.agent.name} is asking for calling tool ${interput.rawItem.providerData} with args ${interput.arguments}`
        );
          if (isAllowed) {
...
main('What is the weather of banglore and send me on pks@gmail.com');